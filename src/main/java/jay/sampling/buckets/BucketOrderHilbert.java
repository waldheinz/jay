/*
 * BucketOrderHilbert.java
 *
 * Created on 3. Juli 2007, 10:51
 */
 ...
 */
public class BucketOrderHilbert implements BucketOrder {
    
    public int[] getSequence(int w, int h) {
        int hi = 0; // hilbert curve index
        int hn = 0; // hilbert curve order
        
        while (((1 << hn) < w || (1 << hn) < h) && hn < 16)
            hn++; // fit to number of buckets
        
        int hN = 1 << (2 * hn); // number of hilbert buckets - 2**2n
        int n = w * h; // total number of buckets
        ...
        for (int i = 0; i < n; i++) {
            ...
            do {
                ...
                hi++;
            } while ((hx >= w || hy >= h || hx < 0 || hy < 0) && hi < hN);
