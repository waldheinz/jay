/*
 * HighContrastMap.java
 *
 * Created on 25. Februar 2006, 14:30
 */
 ...
 */
public class HighContrastMap extends ToneMap {
    
    /** Creates a new instance of HighContrastMap */
    public HighContrastMap() {
    }
...
//        }
    ...
    protected static float C(float y) {
        if (y < 0.0034f)
            return y / 0.0014f;
        else if (y < 1)
            return 2.4483f + (float)Math.log10(y/0.0034f)/0.4027f;
        else if (y < 7.2444f)
            return 16.563f + (y - 1)/0.4027f;
        else
            return 32.0693f + (float)Math.log10(y / 7.2444f)/0.0556f;
    }
    
    protected static float T(float y, float CYmin, float CYmax,
            float maxDisplayY) {
