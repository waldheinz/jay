 ...
 * Color.java
 *
 * Created on 15. Dezember 2005, 17:08
 */
...
public class Spectrum {
    ...
    public static class SpectrumSample implements Comparable {
        public final float w, i;
        
        /**
         * @param wavelength The wavelength in [nm] where this
         *      sample was taken.
         *
         * @param intensity The intensity (or reflectance) measured 
         *      at this wavelength.
         */
        public SpectrumSample(float wavelength, float intensity) {
            this.w = wavelength;
            this.i = intensity;
        }
        
        /** For sorting by wavelengths */
        public int compareTo(Object o) {
            final SpectrumSample ss = (SpectrumSample)o;
            
            if (this.w < ss.w) return -1;
            else if (this.w > ss.w) return 1;
            else return 0;
        }
    ...
    /**
     * Anzahl der Komponenten des Spektrums
     */
    public final static int SAMPLES = 3;
    
    public final static float X_WEIGHT[] = {
        0.412453f, 0.357580f, 0.180423f
    };
    
    public final static float Y_WEIGHT[] = {
        0.212671f, 0.715160f, 0.072169f
    };
    
    public final static float Z_WEIGHT[] = {
        0.019334f, 0.119193f, 0.950227f
    };
    
    public final static float R_WEIGHT[] = {
        3.240479f, -1.537150f, -0.498535f
    };
    public final static float G_WEIGHT[] = {
        -0.969256f,  1.875991f,  0.041556f
    };
    public final static float B_WEIGHT[] = {
        0.055648f, -0.204043f,  1.057311f
    };
    
    /**
     * These are the wavelengths at which the representation
     * samples the true spectral curve. They are chosen to match
     * the colors blue, green and red.
     */
    public final static int WAVELENGTHS[] = { 700, 546, 436 };
    
    public final static Spectrum BLACK = new Spectrum(0.0f);
    public final static Spectrum WHITE = new Spectrum(1.0f);
    
    private final float c[] = new float[SAMPLES];
    
    public Spectrum() {
        this(0.0f);
    }
    
    public Spectrum(float[] cs) {
        for(int i=0; i < SAMPLES; i++) c[i] = cs[i];
    }
    
    public Spectrum(float x) {
        for(int i=0; i < SAMPLES; i++) c[i] = x;
    }
    ...
    public float getMaximum() {
        ...
        for (int i=0; i < SAMPLES; i++) max = Math.max(max, c[i]);
        return max;
    }
    
    /**
     * @deprecated Anzahl der Komponenten ist Variabel
     */
    public Spectrum(float x1, float x2, float x3) {
    ...
    public void xyz(float xyz[]) {
        ...
        for (int i = 0; i < SAMPLES; i++) {
            xyz[0] += X_WEIGHT[i] * c[i];
            xyz[1] += Y_WEIGHT[i] * c[i];
            xyz[2] += Z_WEIGHT[i] * c[i];
        }
    }
    ...
    /**
     * Takes a measured SPD curve and converts it into the
     * internal spectrum representation. Following assumptions
     * are made about the passed arrays: the intensity of the
     * i<i>th</i> sample is in[i], which was measured at
     * wavelength wl[i]. The array wl has to be sorted ascending.
     */
    public static Spectrum fromSamples(float[] wl, float[] in) {
        
        float c[] = new float[SAMPLES];
        
        for (int i=0; i < SAMPLES; i++) {
            int pos = java.util.Arrays.binarySearch(wl, WAVELENGTHS[i]);
            if (pos >= 0) {
                /* exact match */
                c[i] = in[pos];
            } else {
                /* extra- or interpolate */
                pos = -(pos+1);
                if (pos==0) c[i] = in[0];
                else if (pos == in.length) c[i] = in[in.length-1];
                else {
                    /* interpolate */
                    float left = wl[pos-1];
                    float right = wl[pos];
                    float p = (WAVELENGTHS[i] - left) / (right - left);
                    c[i] = p * in[pos-1] + (1-p) * in[pos];
                }
            }
        }
    ...
    public Spectrum scale(float f) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = c[i] * f;
        return ret;
    }
    
    public Spectrum scale(Spectrum c) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = c.c[i] * this.c[i];
        return ret;
    }
    
    
    public Spectrum add(Spectrum c) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = c.c[i] + this.c[i];
        return ret;
    }
    
    public Spectrum sub(Spectrum c) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = this.c[i] - c.c[i];
        return ret;
    }
    
    public Spectrum div(Spectrum c) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = this.c[i] / c.c[i];
        return ret;
    }
     ...
     * Gibt x / this zurück
     */
    public Spectrum invDiv(float x) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = x / this.c[i];
        return ret;
    }
    
    /**
     * Beschränkt den Wertebereich einer Farbe.
     */
    public Spectrum clamp(float min, float max) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = Math.max(min,
              Math.min(max, this.c[i]));
        return ret;
    }
    
    public Spectrum sqrt() {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) ret.c[i] = (float)Math.sqrt(this.c[i]);
        return ret;
    }
    
    public Spectrum pow(final Spectrum e) {
        Spectrum ret = new Spectrum();
        for(int i=0; i < SAMPLES; i++) {
            ret.c[i] = c[i] > 0.0f ? (float)Math.pow(c[i], e.c[i]) : 0.0f;
        }
        return ret;
    }
    
    /**
     * Gibt die Helligkeit zurück.
     */
    public float y() {
        float v = 0.0f;
        for (int i = 0; i < SAMPLES; i++)
    ...
    public static final int CIE_START = 360;
    public static final int CIE_END = 830;
    public static final int CIE_COUNT = CIE_END - CIE_START + 1;
    
    public final static float CIE_X[] = {
    ...
    public final static float CIE_Y[] = {
    ...
    public final static float CIE_Z[] = {
